...
function JoinSession_Event( event )
	Log("DoodleDoug ---- Server : JoinSession_Event");
	savedCharacterGID = event.CharacterGID;
   savedZoneID = GetZoneID( OwnerGID );
 
	Log("DoodleDoug ---- Server: JoinSession event.CharacterGID="..event.CharacterGID);
	ClientManager:AddClient( event.CharacterGID );
	ClientManager:RunClientProcess( event.CharacterGID, "DoodleDoug/Client.lua" );
...
function ClientProcessEnd_Event( event )
   ...
   Log("DOODLE DIG sending ClientProcessEnd msg");
end
...
function main()
DEBUG = true;
Log("Starting DoodleDoug Server main()");
	ClientManager = SessionManager();
	local SessionID = RegisterSession( GetZoneID( OwnerGID ), 0, "DoodleDoug", 0, 1, "Rules" );
	if DEBUG == true then
		Log( "DoodleDoug server session " .. SessionID .. " active." );
	end
	...
	while true do
		...
		if event then
			if DEBUG == true then
				--Log("Event name : <"..event.MsgName..">");
				if(event.MsgName ~= Messages.Moved) then -- Don't log the movement messages
